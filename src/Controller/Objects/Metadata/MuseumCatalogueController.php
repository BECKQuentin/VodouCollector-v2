...
class MuseumCatalogueController extends AbstractController
{
    ...
    #[IsGranted("ROLE_ADMIN", message: "Seules les ADMINS peuvent faire ça")]
    public function deleteMuseumCatalogue(MuseumCatalogue $metadata, Request $request): Response
    {
            ...
            if ($confirmForm->isSubmitted() && $confirmForm->isValid()) {
                if ($confirmForm->get('confirm')->isClicked()) {
                    foreach ($objects as $object) {
                        ...
                        if  ($this->museumCatalogueRepository->find(1)) {
                            $object->setTypology($this->museumCatalogueRepository->find(1));
                        } else {
